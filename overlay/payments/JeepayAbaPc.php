...
class JeepayAbaPc
{
    ...
    public function form()
    {
        return [
            ...
            'way_code' => ['label' => 'wayCode', 'description' => 'ABA_PC', 'type' => 'input'],
            'settle_currency' => ['label' => 'ABA结算货币', 'description' => 'USD 或 KHR', 'type' => 'input'],
            'cny_to_settle_rate' => ['label' => '人民币→结算币汇率', 'description' => 'USD约0.14；KHR约560', 'type' => 'input'],
    ...
    public function pay($order)
    {
        ...
        $rate = (float) ($this->config['cny_to_settle_rate'] ?? 0.14);
        if ($rate <= 0) {
            throw new ApiException('汇率配置无效');
        }
        $currency = strtoupper(trim((string) ($this->config['settle_currency'] ?? 'USD')));
        if (!in_array($currency, ['KHR', 'USD'], true)) {
            $currency = 'USD';
        }
        if ($currency === 'KHR') {
            $major = max(1, (int) ceil($cny * $rate));
            $jeepayAmount = $major * 100;
            $majorText = (string) $major;
        } else {
            $major = max(0.01, round($cny * $rate, 2));
        ...
        $base = [
            ...
            'wayCode' => (string) ($this->config['way_code'] ?? 'ABA_PC'),
            'amount' => $jeepayAmount,
            'currency' => $currency,
            'subject' => sprintf('%s %s CNY(≈%s %s)', $this->config['product_name'] ?? 'XBoard', number_format($cny, 2, '.', ''), $majorText, $currency),
            'body' => sprintf('CNY %s @ %s = %s %s', number_format($cny, 2, '.', ''), $rate, $majorText, $currency),
        ...
        $payData = $this->unifiedOrReuse($gateway, $appSecret, $base, (string) $order['trade_no']);
        if ($payData === '') {
    ...
    private function unifiedOrReuse($gateway, $appSecret, $base, $tradeNo)
    {
        ...
        if ($exists) {
            ...
            try {
                ...
                Log::warning('[JeepayAbaPc] query fail', ['err' => $e->getMessage()]);
            }
